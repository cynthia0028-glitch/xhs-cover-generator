...
不要做成九宫格，不要做成强营销海报，不要使用超大爆款字体，不要使用固定橙红或粉色装饰，不要过度涂鸦，不要赛博朋克，不要电商感，不要过度锐化，不要过度美颜，不要改变人物身份，不要生成无关人物，不要让文字遮挡人物脸部，不要让三张照片风格割裂。`,
  impact: "大字冲击封面，标题极大，占据画面核心，粗马克笔或刷字质感，高对比、强记忆点、构图夸张，可以使用仰拍、俯拍或广角冲击感，适合教程、避坑、经验总结。文字颜色必须跟随照片取色，不要固定使用某一种爆款色。",
  guofeng: "古典国风封面，结合用户照片与东方审美元素，使用宣纸肌理、墨迹、印章、留白、山水纹样、窗棂、书法感标题或竖排文字。色彩从照片中提取并向墨色、米白、朱砂、青绿、黛蓝、金色等传统色延展。整体要有古典、诗意、雅致和故事感，不要做成廉价古风贴纸。"
};
...
async function generateAiCover() {
  ...
  const images = await Promise.all(uploadedFiles.map(fileToDataUrl));
  ...
  if (!response.ok) {
    ...
    throw new Error(detail || "AI image generation failed");
  }
